...
class CCDevice extends EventEmitter {
  constructor(bus, config) {
    ...
    this.bus.port.on("close", () => {
      this.onBusClosed();
    });
    this.bus.port.on("open", () => {
      this.onBusOpen();
    });
  ...
  exec(cmd, data) {
    ...
    return this.sendCommand(cmd, data).catch((e) => {
      this.emit("error", e);
      throw e;
    });
  }
  sendCommand(command, data) {
    ...
    var newCmd = new this.bus.Command(
      this.config.src,
      this.config.dest,
      command,
      data,
        ...
        switch (type) {
          ...
          case this.eventCodes.escrow:
            if (channel === 0) {
              ...
              this.exec("routeBill", new Uint8Array([0])).catch((e) =>
                console.log(e)
              );
            } else {
              debug("cctalk::device::events::type::routeBill")(
                coin,
                "routeBill"
              );
              //this.emit(this.eventCodes[type], channel);
              this.exec("routeBill", new Uint8Array([1])).catch((e) =>
                console.log(e)
              );
            }
            break;
          ...
          default:
            ...
            this.emit(
              "error",
              new Error(
                "The device reported a malfunction: Code " +
                  type +
                  ", " +
                  channel
              )
